...
Pull requests get a preview deployment with its own Convex preview backend, recreated empty on each push by `scripts/vercel-build.sh`; see the header of that script for the Vercel Preview environment variables it needs.
